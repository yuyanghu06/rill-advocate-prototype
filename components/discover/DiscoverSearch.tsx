...
export default function DiscoverSearch({ isRecruiter = false }: { isRecruiter?: boolean }) {
  ...
  // Load all profiles on mount (browse mode)
  useEffect(() => {
        ...
        {filtersOpen && (
          <aside className="w-60 flex-shrink-0 bg-white border-r border-slate-100 overflow-y-auto px-4 py-4 space-y-5">
            ...
            <div>
              <label className="text-xs font-medium text-slate-600 block mb-1.5">
                Min credibility:{" "}
                <span className="text-slate-800">{filters.min_ranking}</span>
              </label>
              <input
                type="range"
                min={0}
                ...
                onChange={(e) =>
                  handleFilterChange({ min_ranking: parseInt(e.target.value) })
                }
                className="w-full accent-brand-500"
              />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                ...
                <button
                  onClick={addSkill}
                  className="text-xs bg-slate-100 hover:bg-slate-200 px-2.5 py-1.5 rounded-lg transition-colors"
                >
                  Add
                </button>
              </div>
                ...
                <div key={key} className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 rounded-full transition-colors flex-shrink-0 ${
                      filters[key] ? "bg-brand-500" : "bg-slate-200"
                    }`}
                  >
                    <span
